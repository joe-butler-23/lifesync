import { createContext, createElement, Fragment, memo, Children, useContext, useEffect, useRef, useReducer, useCallback } from 'react';
import classNames from 'classnames';
import { CompositeDecorator, ContentBlock, genKey, EditorState, ContentState, SelectionState, EditorBlock, Modifier, RichUtils, Editor as Editor$1, KeyBindingUtil, getDefaultKeyBinding } from 'draft-js';
...
function MenuContainer({
  ...
  })), createElement(Menu, null, () => createElement(Fragment, null, createElement(MenuButton, null, createElement("button", {
    className: classNames(buttonStyles['icon-button'], buttonStyles['button']),
    "aria-label": "Show menu"
  }, createElement(DotHorizontal, {
    className: styles['menu-icon']
  }))), createElement(MenuList, {
    className: styles['slide-down']
  }, createElement(MenuItem, {
    onSelect: onExpandAllClick
  }, "Expand all"), createElement(MenuItem, {
...
var Hashtag$1 = /*#__PURE__*/memo(Hashtag);
...
function Link(props) {
  const link = Children.map(props.children, child => child && child.props ? child.props.text : '').join('');
...
function getEmptySlateState(rootId) {
  const firstItem = getEmptyBlock().set('depth', 1).set('data', Immutable.Map({
    parentId: rootId,
    pos: getPosNum(1)
  }));
  ...
  let emptySlate = EditorState.createWithContent(ContentState.createFromBlockArray(firstBlocks), createDecorators());
  emptySlate = EditorState.forceSelection(emptySlate, SelectionState.createEmpty(firstItem.getKey()));
  return emptySlate;
}
...
function appendChild(blockMap, parentBlockKey, blockToAdd) {
  ...
  return insertBlocksAfter(blockMap, OrderedMap({
...
function addEmptyBlockToEnd(editorState, zoomedInItemId, depth) {
  ...
  return EditorState.forceSelection(EditorState.push(editorState, newContentState, 'add-new-item'), newSelection);
}
...
function recreateParentBlockMap(contentState, blockMap, parentId) {
  ...
  const recreatedContentState = ContentState.createFromBlockArray(makeCorrectionsToNodeAndItsDescendants(blockMap, blockMap.get(parentId)));
...
function Disc({
  ...
  }, isCollapsible && createElement("button", {
...
function PlusSign(props) {
  return createElement("svg", Object.assign({
    ...
    xmlSpace: "preserve"
  }, props), createElement("g", null, createElement("g", null, createElement("path", {
...
function MinusSign(props) {
  return createElement("svg", Object.assign({
    ...
    enableBackground: "new 0 0 512 512",
    xmlSpace: "preserve"
  }, props), createElement("g", null, createElement("rect", {
    y: 236,
    width: 512,
    height: 40
  })));
...
function wrapInNestedDivs(el, props, depth) {
  ...
  return createElement('div', props, wrapInNestedDivs(el, props, depth - 1));
}
...
const Item = /*#__PURE__*/memo(props => {
  const {
    block,
    blockProps
  } = props;
  ...
  return wrapInNestedDivs(createElement("div", {
    className: itemClasses,
    style: {
      marginLeft: -13
    },
    "data-testid": "list-item"
  }, block.getDepth() > baseDepth && createElement(Disc, {
    itemId: block.getKey(),
    collapsed: !!collapsed,
    isCollapsible: collapsible
  }), createElement("div", {
    className: styles$3['item-container']
  }, createElement(EditorBlock, Object.assign({}, props)), collapsible ? !collapsed ? createElement("button", {
    contentEditable: false,
...
function pasteText(editorState, pastedText) {
  ...
  return EditorState.push(editorState, Modifier.replaceText(contentState, selectionState, pastedText), 'paste-text');
}
...
function moveCurrentBlockUp(editorState, zoomedInItemId) {
  ...
  const newState = EditorState.push(editorState, newContentState, 'move-block');
...
function toggleCollapseState(editorState, collapseState, blockKey) {
  ...
  const newState = EditorState.push(editorState, newContentState, collapseState ? 'collapse-list' : 'expand-list');
  return EditorState.forceSelection(newState, newSelection);
}
...
function toggleCompleteReducer(state) {
  ...
  return { ...state,
    editorState: EditorState.push(state.editorState, newContentState, 'toggle-completion')
  };
}
...
function deleteItemWithChildren(state) {
  ...
  if (pSibling) {
    ...
    newSelection = SelectionState.createEmpty(newBlockMap.toSeq().first().getKey());
  } // reset the hasChildren state of the parent block


  newBlockMap = adjustHasChildren(newBlockMap, parentId);
  return { ...state,
    editorState: EditorState.push(editorState, contentState.merge({
      blockMap: newBlockMap,
      selectionBefore: newSelection,
...
function expandCollapseAll(state, collapse) {
  ...
  const newEditorState = EditorState.push(editorState, contentState.set('blockMap', newBlockMap), 'collapse-all');
  return { ...state,
...
function toggleBookmark(editorState, zoomedInItemId) {
  ...
  return EditorState.push(editorState, contentState.set('blockMap', newBlockMap), 'bookmark');
}
...
function usePrevious(value) {
  ...
  const ref = useRef(undefined); // Store current value in ref
...
function sanitizePosAndDepthInfo(editorState, rootKey) {
  ...
  const newEditorState = EditorState.createWithContent(ContentState.createFromBlockArray(blocks), createDecorators());
  let selection;
...
function forceUpdateEditorState(editorState, searchText = '') {
  ...
  const newEditorStateInstance = EditorState.createWithContent(editorState.getCurrentContent(), createDecorators(searchText));
...
const ListNestingStyles = /*#__PURE__*/memo(props => {
  const {
    max
  } = props;
  return max > DRAFT_DEFAULT_MAX_DEPTH ? createElement("style", null, generateListNestingStyles(`${CUSTOM_DEPTH_CLASS}`, max)) : null;
});
...
function DeepnotesEditor(props) {
  ...
  const editor = useRef(null); // using useReducer instead of multiple useState calls because this
  ...
  const [state, dispatch] = useReducer(rootReducer, // The second argument to useReducer can also be thought as the default
  ...
  const expandParticularBlock = useCallback(blockKey => {
    dispatch({
      type: EXPAND_ITEM,
      blockKey
    });
  }, [dispatch]);
  const collapseParticularBlock = useCallback(blockKey => {
    dispatch({
      type: COLLAPSE_ITEM,
      blockKey
    });
  }, [dispatch]);
  const zoomTo = useCallback(blockKey => {
    dispatch({
      type: ZOOM,
      blockKey
    });
  ...
  const updateEditorState = useCallback(debounce((es, searchText) => {
  ...
  function handleChange(editorState) {
    ...
    if (anchorBlock && anchorBlock.getType() !== 'unordered-list-item') {
      const newContentState = Modifier.setBlockType(contentState, selectionState, 'unordered-list-item'); // TODO: What should be the change type here?

      newState = EditorState.push(editorState, newContentState, 'force-list-type');
    }
  ...
  }, withToolbar && createElement(MenuContainer, {
  ...
  }), createElement(ListNestingStyles, {
    max: 40
  })), createElement("button", {
...
var Editor = /*#__PURE__*/memo(DeepnotesEditor);

export default Editor;
export { ROOT_KEY, createDecorators, findParent, pluckGoodies };
//# sourceMappingURL=deepnotes-editor.esm.js.map
