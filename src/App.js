...
import { Calendar, CheckSquare, Utensils, Dumbbell, MessageCircle, Plus, Users, GripVertical, ChevronLeft, ChevronRight } from 'lucide-react';
...
const LifeDashboardApp = () => {
  ...
  const [googleCalendarToken, setGoogleCalendarToken] = useState(localStorage.getItem('googleCalendarToken') || '');
  ...
  }, [currentWeekStart, setLoadingGoogleCalendarEvents, setGoogleCalendarError, setGoogleCalendarEvents]);
  ...
  const handleUpdateCalendarEvent = async (eventId, eventData) => {
    ...
    try {
      const updatedEvent = await GoogleCalendarService.updateEvent(googleCalendarToken, eventId, eventData);
  ...
  const handleDeleteCalendarEvent = async (eventId) => {
    ...
    try {
      ...
      setGoogleCalendarEvents(prev => prev.filter(event => event.id !== eventId));
    } catch (error) {
      setGoogleCalendarError(error);
  ...
  const handleTaskDrop = useCallback(async (taskId, targetDate, insertPosition = null) => {
    ...
    const newDueDate = targetDate ? targetDate.toISOString().split('T')[0] : null;
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  { 
                    key: 'bridge_club', 
                    label: 'Bridge Club', 
                    count: tasks.filter(t => t.project_name && t.project_name.toLowerCase().includes('bridge club')).length 
                  },
                  { 
                    key: 'home', 
                    label: 'Home', 
                    count: tasks.filter(t => t.project_name && t.project_name.toLowerCase() === 'home').length 
                  },
                  { 
                    key: 'cooking', 
                    label: 'Cooking', 
                    count: tasks.filter(t => t.project_name && t.project_name.toLowerCase() === 'meal_planning').length 
                  }
                ].map(filter => (
                  <button
                    key={filter.key}
  ...
  const renderDashboard = () => {
    ...
    const dayRecipes = scheduledRecipes[selectedDateKey] || { lunch: [], dinner: [] };
    ...
    const getFilteredDayTasks = () => {
      ...
      switch (dayTaskFilter) {
        ...
        case 'uncategorised':
          filtered = dayTasks.filter(task => 
            !task.project_name || task.project_name.trim() === ''
          );
          break;
        case 'all':
        default:
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-3 min-h-[120px]">
                {dayWorkouts.length === 0 ? (
                  ...
                  dayWorkouts.map(workoutId => {
                    ...
                    return workout ? (
                      <div key={workout.id} className="p-3 bg-green-50 rounded-lg border border-green-200">
                        <h4 className="font-medium text-green-900">{workout.name}</h4>
                        ...
                        <p className="text-xs text-green-600 mt-1">{workout.equipment}</p>
                      </div>
                    ) : null;
                  })
                )}
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border">
              ...
              <div className="space-y-4">
                {/* Lunch */}
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-2">🍽️ Lunch</h4>
                  <div className="space-y-2 min-h-[60px]">
                    {dayRecipes.lunch.length === 0 ? (
                      ...
                      dayRecipes.lunch.map(recipeId => {
                        ...
                        return recipe ? (
                          <div key={recipe.id} className="p-2 bg-orange-50 rounded-lg border border-orange-200">
                            <h5 className="font-medium text-orange-900 text-sm">{recipe.name}</h5>
                ...
                {/* Dinner */}
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-2">🍽️ Dinner</h4>
                  <div className="space-y-2 min-h-[60px]">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-gray-500">{filteredDayTasks.length} tasks</span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                >
                  Bridge Club ({dayTasks.filter(t => t.project_name && t.project_name.toLowerCase().includes('bridge club')).length})
                </button>
                <button
                ...
                >
                  Home ({dayTasks.filter(t => t.project_name && t.project_name.toLowerCase() === 'home').length})
                </button>
                <button
                    ...
                    <div key={task.id} className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-start justify-between">
                        <div className="flex items-start">
                          ...
                          />
                          <div>
                            <p className={`font-medium ${task.completed ? 'line-through text-gray-500' : 'text-blue-900'}`}>
                              {task.content || task.title}
                            </p>
        ...
        {(provided) => (
          <div
            ...
            className={`p-2 rounded-md cursor-move group transition-colors ${
              isScheduled ? 'bg-red-100 hover:bg-red-200' : 'bg-gray-100 hover:bg-gray-200'
            }`}
          >
            <div className="flex items-center justify-between">
              <div>
                <p className={`text-xs font-medium ${isScheduled ? 'text-red-900' : 'text-gray-900'}`}>
                  {task.content}
                </p>
                <div className="flex items-center text-xs mt-1 space-x-1">
                  {task.priority && (
                    <span className={`px-1 py-0.5 rounded-full ${
                      isScheduled ? 'bg-red-200 text-red-700' :
                      task.priority === 4 ? 'bg-red-100 text-red-600' :
                      task.priority === 3 ? 'bg-yellow-100 text-yellow-600' :
                      task.priority === 2 ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'
                    }`}>
                      P{task.priority}
                    </span>
                  )}
      ...
      <Draggable draggableId={String(recipe.id)} index={index}>
        {(provided) => (
          <div
            ...
            {...provided.dragHandleProps}
            className={`p-2 rounded-md cursor-move group transition-colors ${
              isScheduled ? 'bg-orange-100 hover:bg-orange-200' : 'bg-gray-100 hover:bg-gray-200'
            }`}
          >
            <div className="flex items-center justify-between">
              <div>
                <p className={`text-xs font-medium ${isScheduled ? 'text-orange-900' : 'text-gray-900'}`}>
                  {recipe.name}
                </p>
                <div className="flex items-center text-xs mt-1 space-x-1">
                  <span className={`px-1 py-0.5 rounded-full ${
                    isScheduled ? 'bg-orange-200 text-orange-700' : 'bg-gray-200 text-gray-600'
                  }`}>
                    {recipe.cookTime}
                  </span>
                  <span className={`px-1 py-0.5 rounded-full ${
      ...
      <Draggable draggableId={String(workout.id)} index={index}>
        {(provided) => (
          <div
            ...
            {...provided.dragHandleProps}
            className={`p-2 rounded-md cursor-move group transition-colors ${
              isScheduled ? 'bg-green-100 hover:bg-green-200' : 'bg-gray-100 hover:bg-gray-200'
            }`}
          >
            <div className="flex items-center justify-between">
              <div>
                <p className={`text-xs font-medium ${isScheduled ? 'text-green-900' : 'text-gray-900'}`}>
                  {workout.name}
                </p>
                <div className="flex items-center text-xs mt-1 space-x-1">
                  <span className={`px-1 py-0.5 rounded-full ${
                    isScheduled ? 'bg-green-200 text-green-700' : 'bg-gray-200 text-gray-600'
                  }`}>
                    {workout.duration}
                  </span>
                  <span className={`px-1 py-0.5 rounded-full ${
            ...
            >
              <div className="space-y-2">
                {dayTasks.map((task, index) => (
                  <DraggableTask key={task.id} task={task} isScheduled={true} index={index} />
                ))}
                {dayEvents.map(event => (
                  <Draggable key={event.id} draggableId={`event-${event.id}`} index={dayTasks.length + dayEvents.indexOf(event)}>
                    {(provided) => (
                      <div
                        ref={provided.innerRef}
                        {...provided.draggableProps}
        ...
        <div className="bg-gray-50 rounded-lg p-2 min-h-[300px] space-y-3">
          ...
          <Droppable droppableId={`${dateKey}-lunch`}>
            {(provided) => (
              ...
              >
                <h4 className="text-xs font-semibold text-orange-800 mb-2">🍽️ Lunch</h4>
                <div className="space-y-1">
                  {dayRecipes.lunch.map((recipeId, index) => {
                    const recipe = mockRecipes.find(r => r.id === recipeId);
                    return recipe ? (
                      <DraggableRecipe key={recipe.id} recipe={recipe} isScheduled={true} index={index} />
                    ) : null;
                  })}
                </div>
                {provided.placeholder}
          ...
          <Droppable droppableId={`${dateKey}-dinner`}>
            {(provided) => (
              <div
                ...
                className="bg-white rounded-md p-2 min-h-[120px] border-2 border-dashed border-orange-200 hover:border-orange-300 transition-colors"
              >
                <h4 className="text-xs font-semibold text-orange-800 mb-2">🍽️ Dinner</h4>
                <div className="space-y-1">
            ...
            >
              <div className="space-y-2">
                {dayWorkouts.map((workoutId, index) => {
                  const workout = mockWorkouts.find(w => w.id === workoutId);
                  return workout ? (
                    <DraggableWorkout key={workout.id} workout={workout} isScheduled={true} index={index} />
                  ) : null;
                })}
              </div>
              {provided.placeholder}
                ...
                >
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {mockWorkouts.map((workout, index) => (
                      <DraggableWorkout key={workout.id} workout={workout} index={index} />
                    ))}
                  </div>
                  <div className="mt-4 pt-4 border-t">
                    <p className="text-xs text-gray-500">
          ...
          return (
            <div className="w-80 bg-white rounded-xl p-4 shadow-sm border">
              ...
              {overdueTasks.length > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4" key="overdue-tasks-box">
                  <h3 className="font-semibold text-red-800 mb-2">Overdue Tasks ({overdueTasks.length})</h3>
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {overdueTasks.map((task, index) => (
                      <div key={task.id} className="bg-red-100 p-2 rounded-md">
                        <p className="text-xs font-medium text-red-900">{task.content}</p>
                        <p className="text-xs text-red-600">Due: {new Date(task.due).toLocaleDateString()}</p>
                      </div>
                    ))}
                  </div>
                </div>
               ...
               <div className="mb-4">
                 ...
                 <div className="flex flex-wrap gap-2" key="task-filters-buttons">
                   <button
                     onClick={() => setTaskFilter('all')}
                     className={`px-3 py-1 rounded-full text-xs font-medium ${
                       taskFilter === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                     }`}
                   >
                   ...
                   <button
                     ...
                     className={`px-3 py-1 rounded-full text-xs font-medium ${
                       taskFilter === 'bridge_club' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                     }`}
                   >
                     Bridge Club ({tasks.filter(t => t.project_name && t.project_name.toLowerCase().includes('bridge club') && !t.completed).length})
                   </button>
                   <button
                     onClick={() => setTaskFilter('home')}
                     className={`px-3 py-1 rounded-full text-xs font-medium ${
                       taskFilter === 'home' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                     }`}
                   >
                     Home ({tasks.filter(t => t.project_name && t.project_name.toLowerCase() === 'home' && !t.completed).length})
                   </button>
                   <button
                     onClick={() => setTaskFilter('urgent')}
                     className={`px-3 py-1 rounded-full text-xs font-medium ${
                       taskFilter === 'urgent' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                     }`}
                   >
                     Urgent ({tasks.filter(t => t.priority === 4 && !t.completed).length})
                   </button>
                 </div>
               </div>
                 ...
                 {(provided) => (
                   ...
                   >
                     ...
                     {unscheduledTasks.length === 0 ? (
                       <p className="text-gray-500 text-sm">No unscheduled tasks</p>
                     ) : (
                       unscheduledTasks.map((task, index) => (
                         <DraggableTask key={task.id} task={task} index={index} />
                       ))
      ...
      return (
        <div className="bg-gray-50 rounded-lg p-2 min-h-[300px] space-y-3">
          ...
          <div className="bg-blue-50 rounded-md p-2 min-h-[80px] border-2 border-dashed border-blue-200">
            <h4 className="text-xs font-semibold text-blue-800 mb-2">🗓️ Events</h4>
            <div className="space-y-1">
              {dayEvents.map(event => (
          ...
          <Droppable droppableId={`${dateKey}-tasks`}>
            {(provided) => (
              ...
              >
                <h4 className="text-xs font-semibold text-blue-800 mb-2">✅ Tasks</h4>
                <div className="space-y-1">
                  {dayTasks.map((task, index) => (
                    <DraggableTask key={task.id} task={task} isScheduled={true} index={index} />
                  ))}
          ...
          <Droppable droppableId={`${dateKey}-workouts`}>
            {(provided) => (
              ...
              >
                <h4 className="text-xs font-semibold text-green-800 mb-2">💪 Workouts</h4>
                <div className="space-y-1">
                  {dayWorkouts.map((workoutId, index) => {
                    const workout = mockWorkouts.find(w => w.id === workoutId);
                    return workout ? (
      ...
      switch (plannerMode) {
        case 'recipes':
          return weekDates.map((date) => {
            const dateKey = date.toISOString().split('T')[0];
            return <RecipeModeDay key={dateKey} date={date} dayName={date.toLocaleDateString('en-US', { weekday: 'short' })} />;
          });
        case 'workouts':
          return weekDates.map((date) => {
            const dateKey = date.toISOString().split('T')[0];
            return <WorkoutModeDay key={dateKey} date={date} dayName={date.toLocaleDateString('en-US', { weekday: 'short' })} />;
          });
        case 'all':
          return weekDates.map((date) => {
            const dateKey = date.toISOString().split('T')[0];
            return <AllModeDay key={dateKey} date={date} dayName={date.toLocaleDateString('en-US', { weekday: 'short' })} />;
          });
        default:
          return weekDates.map((date) => {
            const dateKey = date.toISOString().split('T')[0];
            return <TaskModeDay key={dateKey} date={date} dayName={date.toLocaleDateString('en-US', { weekday: 'short' })} />;
          });
      }
    };
  ...
  const onDragEnd = React.useCallback((result) => {
            ...
            if (newScheduled[dateKey] && typeof newScheduled[dateKey] === 'object') {
              Object.keys(newScheduled[dateKey]).forEach(mealType => {
                if (Array.isArray(newScheduled[dateKey][mealType])) {
                  newScheduled[dateKey][mealType] = newScheduled[dateKey][mealType].filter(
                    id => id !== draggableId
                  );
                }
              });
            ...
            if (newScheduled[existingDateKey] && typeof newScheduled[existingDateKey] === 'object') {
              Object.keys(newScheduled[existingDateKey]).forEach(existingMealType => {
                if (Array.isArray(newScheduled[existingDateKey][existingMealType])) {
                  newScheduled[existingDateKey][existingMealType] = newScheduled[existingDateKey][existingMealType].filter(
                    id => id !== draggableId
                  );
                }
              });
    ...
    if (plannerMode === 'all') {
      ...
      if (draggableId.startsWith('r')) { // It's a recipe
        ...
        return;
      } else if (draggableId.startsWith('w')) { // It's a workout
        if (destination.droppableId === 'unscheduled-all') {
          // Dragged back to sidebar - remove from schedule
          setScheduledWorkouts(prev => {
            const newScheduled = { ...prev };
        ...
        try {
          ...
          console.error('Invalid date for workout scheduling in all mode:', destination.droppableId);
        }
        return;
      } else { // It's a task
        if (destination.droppableId === 'unscheduled-all') {
        ...
        try {
          ...
          if (isNaN(destDate.getTime())) {
            console.error('Invalid date for task scheduling in all mode:', destination.droppableId);
            return;
          }
